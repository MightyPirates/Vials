                ...
                if (fluidHandlerItem != null) {
                    ...
                    if (tankProperties.length != 0) {
                        ...
                        if (contents != null && contents.amount > 0) {
                            FluidActionResult result = FluidUtil.tryEmptyContainerAndStow(stack, fluidHandlerDrain, playerInventory, tankProperties[0].getCapacity(), player);
                            if (result.isSuccess()) {
                                player.setHeldItem(hand, result.getResult());
                            }
                        } else {
                            FluidActionResult result = FluidUtil.tryFillContainerAndStow(stack, fluidHandlerDrain, playerInventory, tankProperties[0].getCapacity(), player);
                            if (result.isSuccess()) {
                                player.setHeldItem(hand, result.getResult());
                            }
                        }
